...
export default function CompetencyGap() {
  ...
  useEffect(() => {
    const animateCount = (id, target, duration, suffix = '', isFloat = false) => {
    ...
    >
      ...
      <div className="mb-5 grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-[10px]">
        {stats.map(({ id, label }) => (
          <div
            key={id}
            className="rounded-md border border-[rgba(184,146,42,0.12)] bg-[#1A1714] p-3 text-center sm:p-[14px]"
          >
            <span
              id={id}
      ...
      >
        <p className="m-0 text-[12px] leading-relaxed text-[#7A7068] sm:text-[12.5px]">
          ...
          competitors by <span className="font-medium text-gold-light">3–4×</span>. The gap
          is the opportunity — and it closes{' '}
